...
""" Dynamic Window transformer for 3D point cloud data """
...
from Dawin3D-L.modules.mink_layers import (
    assign_feats,
    SparseTensorLayerNorm,
    SparseTensorLinear,
)
from Dawin3D-L.sparse_dl.attn.attn_coff import (
    SelfAttnAIOFunction,
    PosEmb,
    TableDims,
    IndexMode,
...
import Dawin3D-L.sparse_dl.knn
from Dawin3D-L.sparse_dl.knn import KNN
...
class SwinTransformerBlock(nn.Module):
    def __init__(
        ...
        self.dim = dim  ## C
        # self.window_size = [window_size, window_size + 2, window_size + 4]
        self.window_size = [window_size, 2 * window_size, 4 * window_size]
        self.num_groups = len(self.window_size)
        
        self.channel = self.dim // self.num_groups
        assert dim == self.channel * self.num_groups
        ...
        self.d = int(self.dim / (2*self.num_groups))
        
        self.fc0 = nn.Linear(self.dim, self.channel)
        self.proj = nn.Linear(self.dim, self.dim)
        self.fc1 = nn.Linear(self.dim, self.d)
        self.fc2 = nn.Linear(self.d, self.dim)
        self.fc3 = nn.Linear(self.channel, self.dim)
        
        self.attn0 = WindowAttention(
                                    dim=self.channel,
    ...
    def forward(self, feats, attn_args):
        ...
        group = self.attn0(feats, attn_args) # [N, C/num_groups]
        groups.append(group)
        
        group = self.attn1(feats, attn_args) # [N, C/num_groups]
        groups.append(group)
        
        group = self.attn2(feats, attn_args) # [N, C/num_groups]
        groups.append(group)
                
        input_feats = torch.cat(groups, dim=1) # [N, C]
        input_groups = input_feats.transpose(0, 1).view(self.num_groups, self.channel, -1)
        
        input_proj = self.proj(input_feats)  ## stored for skip connection
        
        input_feats = self.act_layer(input_proj)
        
        input_feats = input_feats.mean(dim=0, keepdim=True) # [1, C]
        
        y_fuse = self.act_layer(self.fc1(input_feats)) # [1, d]
        
        attn_vectors = self.fc2(y_fuse).view(self.num_groups, self.channel, -1) # [1, d] -> [1, C] -> [num_group, C/num_group, 1]
        
        attn_vectors = self.softmax(attn_vectors) # [num_group, C/num_group, 1]
        
        feats_V = torch.sum(input_groups * attn_vectors, dim=0) # [num_group, C/num_group, N] (broadcasting) -> [C/num_group, N]
        
        feats_V = feats_V.transpose(0, 1) # [N, C/num_group]
        
        y_select = self.fc3(feats_V) # [N, C]
        
        feats = input_proj + y_select # [N, C]
