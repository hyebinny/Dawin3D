...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.resume:
        ...
        if os.path.isfile(args.resume):
            ...
            ## Multi-GPU resume 할 때 추가
            # weights = {f"module.{k}": v for k, v in weights.items()}


            #########################################################################################
            ## Structured3D pretrained weight 활용
            # 'module.backbone.stem_layer.conv_layers.0.kernel' 형식
            # weights = {k.replace('module.backbone.', ''): v for k, v in weights.items()}
            #########################################################################################
